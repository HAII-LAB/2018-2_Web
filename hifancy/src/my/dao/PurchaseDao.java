...
public class PurchaseDao {
	public void insert(Connection conn, Purchase purchase) 
		...
		try {
			...
			("insert into purchase (userId, content ,payment, purchaseDate)"+" values(?,?,?,?)");
			pstmt.setString(1, purchase.getUserId());
			pstmt.setString(2, purchase.getContent());
			pstmt.setInt(3, purchase.getPayment());
	...
	public Purchase selectById(Connection conn, int purchaseId) 
		...
		try {
			...
			if (rs.next()){
				...
				purchase.setContent(rs.getString(3));
				purchase.setPayment(rs.getInt(4));
